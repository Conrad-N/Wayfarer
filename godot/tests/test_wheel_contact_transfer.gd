...
func test_seated_dump_uses_joint_and_preserves_handoff() -> void:
	...
	check(ship.api.set_warp(10.0), "an unsettled restraint no longer refuses warp; the request waits")
	await _frames(2)
	check(flight.ship_is_local and float(flight.session.world.rate) <= 1.0, "accepted warp cannot bypass physical transfer while the wheels unload")
	check(ship.api.set_warp(1.0), "pilot can withdraw the waiting request")
	await _frames(100)
