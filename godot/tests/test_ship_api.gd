...
func test_default_state_and_independence() -> void:
	...
	check_eq(second.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J, "separate battery")
...
func test_door_commands_and_energy() -> void:
	...
	check_eq(api.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J - 1000.0, "one motor draw")
	check(api.set_cargo_door(true), "idempotent open")
	check_eq(api.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J - 1000.0, "repeated open costs nothing")
	check(api.set_cargo_door(false), "close cargo")
	check_eq(api.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J - 2000.0, "closing also costs energy")
	api.consume_energy(ShipApi.BATTERY_CAPACITY_J - 2500.0)
...
func test_airlock_interlock() -> void:
	...
	check_eq(api.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J, "interlocked command costs nothing")
...
func test_door_clearance_validation() -> void:
	...
	check_eq(api.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J, "obstruction costs no energy")
...
func test_resource_accounting_and_exhaustion() -> void:
	...
	check_eq(api.consume_energy(ShipApi.BATTERY_CAPACITY_J - 0.5), ShipApi.BATTERY_CAPACITY_J - 0.5, "normal battery draw")
	...
	powered.consume_energy(ShipApi.BATTERY_CAPACITY_J)
...
func test_invalid_resource_and_damage_requests() -> void:
	...
	check_eq(api.get_telemetry()["battery_energy_j"], ShipApi.BATTERY_CAPACITY_J, "energy unchanged")
