...
func _refresh_ship(data: Dictionary) -> void:
	...
	_readout.text = "SHIP %3.0f%%   BATTERY %.1f kWh\nCARGO %d LOADS / %.1f kg / %.1f m³\n%s" % [float(data.get("ship_health", 0.0)) * 100.0, float(data.get("battery_energy_j", 0.0)) / 3600000.0, manifest.size(), float(data.get("cargo_mass_kg", 0.0)), float(data.get("cargo_volume_m3", 0.0)), solar_line]
