...
- **Milestone 3 — 🟡 early slice.** A **cargo + docked-transfer + station-inventory**
  ...
  budget), now with a **market + credits** layer: docked transfer is a *trade* (load = buy,
  unload = sell), each station prices its goods (static, geographic spread → arbitrage), and
  a ship wallet moves with each trade. Ship-building/refit and **mining** are **not started**.
...
- 🟡 Resources: a **cargo** system exists (a two-limit mass+volume hold; trade at a
  station; inert cargo mass correctly shrinks the Δv budget) and a **credits/market**
  layer — every station prices its goods (buy at the ask, sell at the bid; prices differ
  by station so arbitrage is real) and a ship wallet tracks the money. Prices are static
  for now (supply/demand drift is a later layer). Mining-sourced ore, refined goods, and
  consumables are not modeled yet.
- ⬜ **Mining**: asteroid rendezvous → anchor → extract → haul.
- 🟡 **Stations & docking**: a station with a priced, transferable inventory exists and you
  can dock and **buy/sell** cargo into it. Refuel, repair, and refit are **not started**.
