...
either reports back or proposes a write-action for you to confirm. The prototype
backend is a plain OpenAI-compatible chat-completions loop (in our case a local
Ollama model), so the AI layer is mostly: a persona system prompt + the API
exposed as tools + a confirmation gate on writes.
...
**Provider:** the prototype runs the ship AI on a **local Ollama model** via its
OpenAI-compatible endpoint — no cloud account, no per-token billing, and it's the
same engine that serves the machine's other agents. For a friends server the open
question is whether the server holds one shared model, or each player **brings
their own model/key** (your AI = your machine, your tokens, your limits).
The latter is elegant and scales socially. See
