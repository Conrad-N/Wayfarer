...
The AI panel is a thin bridge: player message + ship-state snapshot → the local
model (with the API as tools) → narration back. Keystones 1 & 2 made concrete.

### 6.1 Tool schema given to the model

In M0 the AI's tools *are* the read API. Each is a standard function tool
definition (OpenAI-compatible `type: "function"` shape); returns are the SI shapes
from §5. The AI converts units and explains; it never recomputes physics.
...
- **AI bridge:** on each player message, snapshot state, run the model's tool-use
  loop (OpenAI-compatible chat completions; prototype: local Ollama), stream narration
  to the console.
