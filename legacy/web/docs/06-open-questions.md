...
An authoritative **Node/TypeScript backend** (deterministic sim + model bridge + later
...
multiplayer (friends open a URL), one language end-to-end, and easy calls to a
local OpenAI-compatible model server (Ollama). (The
...
**Resolved (prototype auth):** the ship AI runs on a **local Ollama model** via its
OpenAI-compatible endpoint (defaults: `qwen3.8:27b-iq4xs` at `http://127.0.0.1:11434/v1`) —
the same engine that serves this machine's coding agent. The bridge is a plain
chat-completions tool loop
([07](07-milestone-0-spec.md) §6 / [src/server/ai-bridge.ts](../src/server/ai-bridge.ts));
no subscription, no per-token billing. Override with `SHIP_AI_BASE_URL` /
`SHIP_AI_MODEL` / `SHIP_AI_API_KEY` in `.env` to point at any OpenAI-compatible server.
