...
- **`.env` used to hold a live OAuth token in a OneDrive-synced folder** (the AI bridge
has since been re-pointed at a local Ollama model — no cloud credential is stored
anymore; `.env` now only carries commented overrides).
- **`.env.example` is missing** though `CLAUDE.md` tells new clones to copy it. Add it with
  commented, valueless keys (`SHIP_AI_BASE_URL`, `SHIP_AI_MODEL`, `SHIP_AI_API_KEY`,
  `SHIP_AI_MAX_TURNS`, `SHIP_AI_TIMEOUT_MS`, `PORT`).
...
- Credential handling is clean: `.env` is gitignored, and (after the Ollama switchover)
  holds no secrets at all — just commented endpoint/model overrides.
