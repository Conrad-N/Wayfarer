...
/** The ship's hold — getCargo. Two limits: mass (kg) and volume (m^3), plus the wallet. */
export interface CargoState {
  ...
  capacityM3: number;
  freeM3: number;
  creditsCr: number; // the operator's wallet [Cr]
}

/** One market row at a station — getStation: a cargo stack plus its prices.
 *  `askCr` = what you pay to buy one; `bidCr` = what you receive selling one. */
export interface MarketItemSummary extends CargoItemSummary {
  askCr: number; // buy price (station → ship)
  bidCr: number; // sell price (ship → station)
}

/** The docked target's market — getStation. `docked:false` when adrift; a target with no
 *  market (a probe) reports docked with hasHold:false + empty inventory. Carries the wallet
 *  so the trade UI can show credits beside the ledger. */
