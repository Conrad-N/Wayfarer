...
import { runShipAI, aiAvailable, PERSONA_LIST, DEFAULT_PERSONA, AI_BASE_URL, AI_MODEL, type ChatMessage } from "./ai-bridge";
...
app.get("/api/ai/status", async (_req, res) =>
  res.json({ available: await aiAvailable(), personas: PERSONA_LIST, defaultPersona: DEFAULT_PERSONA }),
);
...
app.post("/api/ai/chat", async (req, res) => {
  if (!(await aiAvailable())) {
    return res.status(503).json({
      error:
        `AI offline: model server not reachable at ${AI_BASE_URL} (model ${AI_MODEL}). ` +
        "Start Ollama (or set SHIP_AI_BASE_URL to another OpenAI-compatible endpoint) and reload.",
    });
  }
...
app.listen(PORT, () => {
  ...
  console.log(`[server] ship AI: ${AI_MODEL} via ${AI_BASE_URL} (OpenAI-compatible / local Ollama)`);
});
