...
export function getShip(w: World) {
  ...
  return {
    ...
    propellantKg,
    ispSeconds,
    creditsCr: w.creditsCr, // the operator's wallet (docs/02 §Stations)
...
// The fraction of a station's listed price it pays when BUYING from you — the bid/ask spread
// (docs/02 §Stations). Static for the M3 MVP; a station's listed priceCr is its sell/ask.
const SELL_FACTOR = 0.85;

/** What a good costs to buy and what it fetches to sell AT THIS STATION — the station view of
 *  an inventory entry. `askCr` = you pay (the listed price); `bidCr` = you receive (price ×
 *  spread, floored). qty is how many the station has to sell you. */
const marketView = (c: CargoItem) => ({
  ...cargoView(c),
  askCr: c.priceCr ?? 0, // buy price (station → ship)
  bidCr: c.priceCr !== undefined ? Math.floor(c.priceCr * SELL_FACTOR) : 0, // sell price (ship → station)
});
...
export function getCargo(w: World) {
  ...
  return {
    ...
    creditsCr: w.creditsCr, // the wallet the trade debits/credits (docs/02 §Stations)
  };
}
...
export function getStation(w: World) {
  ...
  if (idx === null) {
    return { docked: false as const, index: null, name: null, kind: null, hasHold: false, creditsCr: w.creditsCr, inventory: [] as ReturnType<typeof marketView>[] };
  }
  ...
  return {
    ...
    kind: def.kind,
    hasHold,
    creditsCr: w.creditsCr, // so the trade UI can show the wallet beside the ledger
    inventory: (def.inventory ?? []).map(marketView),
  };
}
...
/** Remove `qty` units of `id` from a hold. The stack is dropped when it empties — UNLESS
 *  `keepEmpty` (a station's priced entry stays at qty 0 so it still defines what it trades). */
function removeFromHold(hold: CargoItem[], id: string, qty: number, keepEmpty = false): void {
  const i = hold.findIndex((c) => c.id === id);
  if (i < 0) return;
  hold[i].qty -= qty;
  if (hold[i].qty <= 0 && !keepEmpty) hold.splice(i, 1);
  else if (hold[i].qty < 0) hold[i].qty = 0;
}

type TransferDir = "load" | "unload"; // load = BUY (station→ship), unload = SELL (ship→station)

/** Trade cargo across the dock — loading is a BUY, unloading is a SELL (docs/02 §Stations).
 *  Validates: docked at a market; the station stocks (buy) / prices (sell) the good; the
 *  ship has room (buy) and you can afford it; and clamps the quantity to whatever binds first
 *  (stock, mass, volume, credits). Moves the wallet and returns the updated holds + credits +
 *  Δv so a caller sees the whole effect of the trade in one round-trip. */
export function transferCargo(w: World, direction: TransferDir, itemId: string, qty = 1) {
  const idx = w.dockedTo;
  if (idx === null) return { ok: false as const, error: "not docked — trading needs a dock" };
  const def = w.targets[idx];
  if (!def.inventory) return { ok: false as const, error: `${def.name} has no market` };
  ...
  if (direction === "load") {
    // BUY: station → ship. Pay the listed price; clamp to stock, hold mass/volume, and credits.
    const item = station.find((c) => c.id === itemId);
    if (!item || item.qty <= 0) return { ok: false as const, error: `no "${itemId}" in stock to buy` };
    const ask = item.priceCr ?? 0;
    ...
    const fitsByCredits = ask > 0 ? Math.floor(w.creditsCr / ask) : Infinity;
    const move = Math.min(want, item.qty, fitsByMass, fitsByVolume, fitsByCredits);
    if (move <= 0) {
      if (fitsByCredits <= 0) return { ok: false as const, error: `not enough credits — ${ask} Cr/unit, you have ${Math.floor(w.creditsCr)}` };
      const limit = fitsByVolume < fitsByMass ? "out of volume" : "out of mass capacity";
      return { ok: false as const, error: `hold full — ${limit}` };
    }
    const totalCr = ask * move;
    w.creditsCr -= totalCr;
    removeFromHold(station, itemId, move, true); // keep the priced entry so it can still buy this back
    addToHold(ship, item, move);
    return tradeResult(w, "load", move, ask, totalCr);
  }

  // SELL: ship → station. The station must PRICE the good (a listed entry, even at qty 0);
  // you receive the bid (price × spread). Clamp to what you actually carry.
  const carried = ship.find((c) => c.id === itemId);
  if (!carried || carried.qty <= 0) return { ok: false as const, error: `no "${itemId}" in your hold to sell` };
  const priced = station.find((c) => c.id === itemId);
  if (!priced || priced.priceCr === undefined) return { ok: false as const, error: `${def.name} doesn't trade ${carried.name}` };
  const bid = Math.floor(priced.priceCr * SELL_FACTOR);
  const move = Math.min(want, carried.qty);
  const totalCr = bid * move;
  w.creditsCr += totalCr;
  removeFromHold(ship, itemId, move);
  addToHold(station, priced, move); // restock the station's existing priced entry
  return tradeResult(w, "unload", move, bid, totalCr);
}

/** The shared success payload for a trade: how much moved, at what unit price, the credit
 *  delta, and the fresh wallet/holds/Δv so the caller re-renders from one round-trip. */
function tradeResult(w: World, direction: TransferDir, moved: number, unitPriceCr: number, totalCr: number) {
  return {
    ok: true as const,
    direction,
    moved,
    unitPriceCr,
    totalCr, // credits paid (load) or received (unload)
    creditsCr: w.creditsCr,
    cargo: getCargo(w),
    station: getStation(w),
    ship: getShip(w),
  };
}
