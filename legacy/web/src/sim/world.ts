...
export interface CargoItem {
  ...
  qty: number;
  // The unit price [Cr] this good trades at HERE — meaningful only on a station's inventory,
  // not on the ship's hold (goods in the ship are just inert mass). A station SELLS at priceCr
  // and BUYS from you at a discount (priceCr × SELL_FACTOR, the spread). A priced entry defines
  // what a station trades even at qty 0, so it can still buy a good it has none of in stock.
  // Static per-station (M3 MVP): the geographic price spread is what makes arbitrage real;
  // supply/demand drift is a later layer (docs/02 §Stations).
  priceCr?: number;
}
...
export class World {
  ...
  // The operator's wallet [Cr] (docs/02 §Stations — buy/sell into a living economy). Truth
  // lives in the sim (Keystone 3); trading across a dock moves it (api.transferCargo).
  creditsCr: number;
  ...
  constructor() {
    ...
    // Two markets with deliberately different price sheets, so a profitable run exists each
    // way (static prices ⇒ arbitrage is geographic, docs/02). A station SELLS a good at priceCr
    // and BUYS it from you at priceCr × SELL_FACTOR (api.ts). A priced entry with qty 0 means
    // "buys this but stocks none" — that's how a station can be a seller for your cargo.
    // Worked loops (SELL_FACTOR 0.85): buy Ore @ Depot 38 → sell @ Kestrel ⌊65·0.85⌋=55 (+17/u);
    // buy Water @ Kestrel 25 → sell @ Depot ⌊40·0.85⌋=34 (+9/u).
    this.targets = [
      {
        ...
        inventory: [
          { id: "water", name: "Water", massKg: 250, volumeM3: 0.3, qty: 8, priceCr: 25 },
          { id: "rations", name: "Rations", massKg: 50, volumeM3: 0.35, qty: 20, priceCr: 55 },
          { id: "parts", name: "Spare Parts", massKg: 120, volumeM3: 0.5, qty: 6, priceCr: 220 },
          { id: "alloy", name: "Alloy Plate", massKg: 300, volumeM3: 0.12, qty: 4, priceCr: 160 },
          { id: "ore", name: "Ore", massKg: 500, volumeM3: 0.22, qty: 0, priceCr: 65 }, // buys ore high
          { id: "machinery", name: "Machinery", massKg: 400, volumeM3: 1.5, qty: 0, priceCr: 760 }, // buys machinery
        ],
      },
      {
        name: "Depot Six",
        ...
        inventory: [
          { id: "ore", name: "Ore", massKg: 500, volumeM3: 0.22, qty: 10, priceCr: 38 },
          { id: "machinery", name: "Machinery", massKg: 400, volumeM3: 1.5, qty: 3, priceCr: 820 },
          { id: "ballast", name: "Ballast", massKg: 250, volumeM3: 0.08, qty: 12, priceCr: 4 },
          { id: "water", name: "Water", massKg: 250, volumeM3: 0.3, qty: 0, priceCr: 40 }, // buys water high
          { id: "parts", name: "Spare Parts", massKg: 120, volumeM3: 0.5, qty: 0, priceCr: 175 }, // buys parts
        ],
      },
    ...
    this.dockedTo = null;
    this.creditsCr = 5000; // starting wallet — enough to bootstrap a first trade run
    this.nextSoi = null;
