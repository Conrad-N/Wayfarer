...
import { planManeuver, planIntercept, planTransferWindow, executeManeuver, selectTarget, getOrbit, getTarget, listTargets, getShip, transferCargo, getCargo, getStation } from "../src/sim/api";
import {
  stepPowered,
  integrateAttitude,
  controlTorqueBody,
...
console.log("\n== Market: buy-low / sell-high arbitrage moves credits and cargo ==");
{
  // Targets: [0] Kestrel (buys ore @ ⌊65·0.85⌋=55), [1] Depot Six (sells ore @ 38). The loop
  // is buy ore at Depot, sell at Kestrel — a positive net the static price spread guarantees.
  const sim = new World();
  const start = sim.creditsCr;
  check("starting wallet is 5000 Cr", start === 5000);

  // Trading needs a dock.
  check("trade while adrift is rejected", !transferCargo(sim, "load", "ore", 1).ok);

  // BUY 5 ore at Depot Six (38 Cr/unit).
  sim.dockedTo = 1;
  const buy = transferCargo(sim, "load", "ore", 5);
  const oreHeld = getCargo(sim).items.find((i) => i.id === "ore")?.qty ?? 0;
  check(
    `bought 5 ore for 190 Cr (wallet ${sim.creditsCr})`,
    buy.ok && buy.moved === 5 && oreHeld === 5 && sim.creditsCr === start - 5 * 38,
  );

  // SELL the 5 ore at Kestrel (bid 55 Cr/unit) — net the round trip positive.
  sim.dockedTo = 0;
  const sell = transferCargo(sim, "unload", "ore", 5);
  const oreLeft = getCargo(sim).items.find((i) => i.id === "ore")?.qty ?? 0;
  check(
    `sold 5 ore for 275 Cr (wallet ${sim.creditsCr}), net +${sim.creditsCr - start}`,
    sell.ok && sell.moved === 5 && oreLeft === 0 && sim.creditsCr === start - 190 + 5 * 55 && sim.creditsCr > start,
  );

  // A station only buys goods it prices: Kestrel doesn't trade Ballast.
  // (First put one ballast aboard from Depot, then try to sell it at Kestrel.)
  sim.dockedTo = 1;
  transferCargo(sim, "load", "ballast", 1);
  sim.dockedTo = 0;
  check("selling a good the station doesn't price is rejected", !transferCargo(sim, "unload", "ballast", 1).ok);

  // A buy-only listing (qty 0) can't be bought from: Depot lists water at qty 0 (it buys water).
  sim.dockedTo = 1;
  check("buying an out-of-stock (buy-only) good is rejected", !transferCargo(sim, "load", "water", 1).ok);
  // ...but you CAN sell water to it — Depot pays ⌊40·0.85⌋=34.
  sim.dockedTo = 0; // grab water cheap at Kestrel first (ask 25)
  const w0 = sim.creditsCr;
  transferCargo(sim, "load", "water", 2);
  sim.dockedTo = 1;
  const wsell = transferCargo(sim, "unload", "water", 2);
  check(
    "water buy@25 / sell@34 nets positive across stations",
    wsell.ok && wsell.moved === 2 && sim.creditsCr === w0 - 2 * 25 + 2 * 34,
  );
}

console.log("\n== Market: a buy clamps to credits, and prices/wallet surface in telemetry ==");
{
  const sim = new World();
  sim.creditsCr = 50; // only enough for one ore at Depot's 38 Cr ask
  sim.dockedTo = 1;
  const buy = transferCargo(sim, "load", "ore", 5); // want 5, can afford 1
  check(`buy clamps to affordable qty (moved ${buy.ok ? buy.moved : "—"}, wallet ${sim.creditsCr})`, buy.ok && buy.moved === 1 && sim.creditsCr === 12);
  // A second buy with too little money is rejected outright.
  check("buy with insufficient credits is rejected", !transferCargo(sim, "load", "ore", 1).ok);
  // getStation exposes ask/bid + the wallet; getShip carries credits too.
  const st = getStation(sim);
  const oreRow = st.inventory.find((i) => i.id === "ore");
  check(
    "getStation reports ask/bid prices + wallet; getShip carries credits",
    !!oreRow && oreRow.askCr === 38 && oreRow.bidCr === Math.floor(38 * 0.85) && st.creditsCr === 12 && getShip(sim).creditsCr === 12,
  );
}
