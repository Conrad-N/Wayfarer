...
> ```
>
> The orbit + nav panel run with no credentials; the ship-AI panel runs on the local
> Ollama model — the same engine that serves this machine — via its OpenAI-compatible
> endpoint (defaults: `qwen3.8:27b-iq4xs` at `http://127.0.0.1:11434/v1`; override with
> `SHIP_AI_BASE_URL` / `SHIP_AI_MODEL` in `.env`).
